...
export function createTests(conn: () => Connection) {
  it('should parse "int2Vector" field (text)', async function () {
    await testParse(conn(), DataTypeOIDs.int2vector,
        [["1", "2", "6"]], [[1, 2, 6]], {columnFormat: DataFormat.text});
  });

  it('should parse "int2Vector" field (binary)', async function () {
    await testParse(conn(), DataTypeOIDs.int2vector, [[2, 4, 6]], [[2, 4, 6]],
  ...
  it('should encode "_int2Vector" param', async function () {
    await testEncode(conn(), DataTypeOIDs._int2vector, [[-1, 5, 6], [100, 150, 200]]);
  });
