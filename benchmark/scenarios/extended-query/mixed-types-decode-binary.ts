...
// Same SQL/row target as Mixed-Type Decode (Text Protocol) - only the
// requested column format differs, so the two scenarios are directly
// comparable for postgrejs (its own text vs. its own binary decode path).
export const MIXED_TYPES_DECODE_BINARY_SCENARIO: ScenarioMeta = {
  ...
  description:
    ...
    'text format codes for every column, with no option to request ' +
    "binary (verified in its own source) - so it isn't benchmarked " +
    "here. pg's binary parser table (`pg-types`) is missing exactly the " +
    'column types this scenario decodes - `varchar`/`json`/`jsonb`/' +
    '`bytea` are unregistered for binary, only a handful of numeric/' +
